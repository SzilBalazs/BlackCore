...
Move stringToMove(const Position &pos, const std::string &s) {
  ...
  if (piece.type == PAWN && pos.getEpSquare() == to) {
    flags = EP_CAPTURE;
             ...
             std::abs((long)squareToRank(from) - (long)squareToRank(to)) == 2) {
...
void uciLoop() {
  ...
  out("option", "name", "Hash", "type", "spin", "default", 16, "min", 1, "max",
      1024);
  out("option", "name", "Threads", "type", "spin", "default", 1, "min", 1,
      "max", 1);
  out("option", "name", "Ponder", "type", "check", "default", "false");
  out("option", "name", "Move Overhead", "type", "spin", "default", 10, "min",
      0, "max", 10000);
  ...
  while (true) {
    ...
    if (command == "isready") {
      ...
      if (tokens[0] == "fen") {
        std::string fen;
        for (unsigned int i = 1; i < tokens.size() && tokens[i] != "moves";
             i++) {
          fen += tokens[i] + ' ';
        }
      ...
      if (searchRunning) {
        out("Search is already running!",
            "To stop it use the \"stop\" command.");
        continue;
      }
      ...
      Move moves[200];
      Move *movesEnd = generateAllMoves(pos, moves);
